...
if(!submitcheck('modsubmit') && !$_GET['fast']) {
    ...
    // test row
		...
		echo '<script>alert("已清理");window.location.href="'.ADMINSCRIPT.'?action=index"</script>';
		exit;
	}
	...
	foreach($threadlist as $thread) {
		...
		if($thread['authorid'] && $thread['author']) {
			$thread['author'] = "<a href=\"?action=members&operation=search&uid=$thread[authorid]&submit=yes\" target=\"_blank\">$thread[author]</a>  <a href=\"?action=members&operation=ban&uid=$thread[authorid]\" target=\"_blank\">禁止用户</a>";
